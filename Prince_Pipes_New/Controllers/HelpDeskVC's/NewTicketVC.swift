...
//  NewOffNet
//
//  Created by Ankit Rana on 04/10/21.
//
...
class NewTicketVC: UIViewController ,UIPickerViewDataSource , UIPickerViewDelegate, UITextViewDelegate{
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        ...
        guard let currentText = txtMobileNo .text else {
            return true
        }
        
        // Calculate the new text after applying the replacement string
        let newText = (currentText as NSString).replacingCharacters(in: range, with: string)
        
        // Check if the new text has more than 10 digits
        if newText.count > 10 {
            return false
        }
    ...
    @IBAction func submitAction(_ sender: Any) {
        ...
        let isValidateMobileNumber = Validation.validaPhoneNumber(phoneNumber: self.txtMobileNo.text!)
